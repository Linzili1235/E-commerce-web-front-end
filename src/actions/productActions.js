...
const fetchOneProduct = (productId) => async dispatch => {
    try {
        const response = await axios.get(`https://api.allorigins.win/get?url=http://api-lulu.hibitbyte.com/product/${productId}?${KEYS_URL}`)
...
const fetchProductsPageSorting = (page, sorting, filters) => async dispatch => {
    try {
        ...
        const response = await axios.post(`https://api.allorigins.win/get?url=http://api-lulu.hibitbyte.com/product/allProducts?sortingId=${sorting}&page=${page}&${KEYS_URL}`, filters)
        const data = response?.data?.rs;
